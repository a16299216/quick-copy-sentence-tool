...
$checks = [ordered]@{
  ...
  admin_rows_use_platform_order = $html.Contains('function adminAnswerItems()') -and $html.Contains('platformSortRank(a.platform) - platformSortRank(b.platform)')
  ...
  writes_normalize_platform_codes = $api.Contains('normalize_platform_answer_order') -and $api.Contains('async function normalizePlatformOrder(')
