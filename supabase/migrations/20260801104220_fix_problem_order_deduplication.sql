...
begin
  ...
  -- Deduplicate active problems before assigning row numbers. Putting
  -- DISTINCT beside row_number() keeps one row per answer because every row
  -- number is unique, which caused duplicate old_code values here.
