alter table public.problem_orders
...
begin
  ...
  insert into tmp_problem_order(old_code, old_position, new_position, search_tags)
  select q.problem_code, q.position, q.position, q.search_tags
  from (
    ...
    from (
      select ai.problem_code,
        ...
        coalesce(
          (select po2.search_tags from problem_orders po2 where po2.problem_code = ai.problem_code limit 1),
          '{}'::text[]
        ) as search_tags
  ...
  insert into problem_orders(problem_code, sort_order, search_tags)
  select t.new_position::text, t.new_position, t.search_tags
  from tmp_problem_order t
